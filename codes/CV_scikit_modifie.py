...
# fonction ajoutée
def _cv_threshold(image: np.ndarray, seuil: float = 0.2, sigma: int = 2) -> np.ndarray:
    
    image_no_nan = np.nan_to_num(image, nan=np.nanmean(image))
    ...
    segmentation_result_boolean = image_gray > seuil
...
def _cv_init_level_set(init_level_set, image, image_shape, dtype=np.float64):
    ...
    if isinstance(init_level_set, str):
        ...
        elif init_level_set == 'threshold': # condition ajoutée
            res = _cv_threshold(image)
        else:
...
def chan_vese(
    ...
    phi = _cv_init_level_set(init_level_set, image, image.shape, dtype=float_dtype)
